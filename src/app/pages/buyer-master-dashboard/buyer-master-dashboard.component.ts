...
@Component({
  selector: "app-buyer-master-dashboard",
  templateUrl: "./buyer-master-dashboard.component.html",
  styleUrls: ["./buyer-master-dashboard.component.css"]
})
export class BuyerMasterDashboardComponent implements OnInit {
  ...
  public Getbuyermaster() {
    this.pomservice.GetBuyerMaster().subscribe(
      data => {
        this.spinner.hide();
        this.buyerlist = data;
        this.dummbuyerlist = data;
        this.count = this.buyerlist.length;
      },
      error => { }
    );
  }
  ...
  public DeleteBuyerMaster(id) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.pomservice.DeleteBuyerMaster(id).subscribe(res => {
          let test = res;
          this.Getbuyermaster();
        });
  ...
  public getglmasterexcel() {
    ...
    this.exportAsExcelFile(hhh, "Buyer master");
  }
  ...
  public GetCountryName(even) {
    ...
    if (even.target.value != 0) {
      this.buyerlist = this.dummbuyerlist.filter(x => x.countryID == this.countryid)
      this.count = this.buyerlist.length;
    }
    else {
      this.Getbuyermaster()
  ...
  public GetStateName(even) {
    ...
    if (even.target.value != 0) {
      ...
      this.buyerlist = this.dummbuyerlist.filter(x => x.stateID  == this.stateid)
      this.count = this.buyerlist.length;
    }
    else {
      this.Getbuyermaster();
  ...
  public GetCityName(even) {
    ...
    if (even.target.value != 0) {
      this.buyerlist = this.dummbuyerlist.filter(x => x.cityID   == this.cityid)
      this.count = this.buyerlist.length;
    }
    else {
      this.Getbuyermaster();

...
@Component({
  selector: 'app-approves-pos',
  templateUrl: './approves-pos.component.html',
  styleUrls: ['./approves-pos.component.css']
})
export class ApprovesPosComponent implements OnInit {
  ...
  public GetAuthorizationMaster() {
    ...
    this.pomservice.GetPurchaseOrderApprovredOrders(this.authorizationid, this.startdate, this.enddate).subscribe(data => {
      this.spinner.hide();
      this.polist = data;
  ...
  public CheckPoExistmail() {
    debugger
    this.pomservice.GetEmailSuccess_log(this.purchaseorderid).subscribe(data => {
      debugger
      if (data.length > 0) {
        debugger
        Swal.fire("Email already sent to the vendor. Do you want to send again")
      }
    }, error => {
  ...
  public sendmail1() {
    ...
    this.pomservice.sendemail1(entity).subscribe(data => {
      ...
      if (data == 'Success') {
        ...
        this.message = data;
        Swal.fire("Email not sent. Please Contact Administrator");
        this.emailattchementurl.length = 0;
        this.emailattchementurl = [];
        this.inserterrorlog();
        this.spinner.hide();
